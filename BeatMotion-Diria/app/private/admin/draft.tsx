    ...
    <ScrollView style={{ flex: 1, padding: 20, backgroundColor: "#000" }}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                if (!draft.recipients.some((r) => r.id === u.id)) {
                  setDraft({ ...draft, recipients: [...draft.recipients, u] });
                }
                setRecipientQuery("");
              }}
              style={{ padding: 10, borderBottomColor: "#555", borderBottomWidth: 1 }}
            >
      ...
      <View style={{ flexDirection: "row", flexWrap: "wrap", marginBottom: 10 }}>
        {draft.recipients.map((r) => (
          <View
            key={r.id}
            style={{ backgroundColor: "#1E90FF", padding: 5, margin: 2, borderRadius: 5 }}
          >
      ...
      <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 20 }}>
        <TouchableOpacity
          ...
          style={{ backgroundColor: "#1E90FF", padding: 12, borderRadius: 10, flex: 1 }}
        >
      ...
      {/* Borradores locales */}
      <Text style={{ fontSize: 20, color: "#fff", marginBottom: 10 }}>Borradores Guardados</Text>
      {draftsLocal.map((d) => (
        <View
          key={d.id}
          style={{ backgroundColor: "#222", padding: 10, borderRadius: 10, marginBottom: 8 }}
        >
          <Text style={{ color: "#fff", fontWeight: "bold" }}>{d.title}</Text>
          <Text style={{ color: "#ccc" }}>
            Destinatarios: {d.recipients.map((r) => r.email).join(", ") || "—"}
          </Text>

          <View style={{ flexDirection: "row", marginTop: 5 }}>
            <TouchableOpacity onPress={() => editDraftLocal(d)} style={{ marginRight: 10 }}>
              <Text style={{ color: "#1E90FF" }}>Editar</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => deleteDraftLocal(d.id)} style={{ marginRight: 10 }}>
